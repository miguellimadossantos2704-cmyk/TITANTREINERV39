...
// Elements - Routines
const routinesDashboard = document.getElementById('routinesContent'); // Changed ID to match HTML update
...
function renderRoutines() {
    ...
    routines.forEach(routine => {
        ...
        // Translated "Exercises" count label
        const exerciseLabel = t('exercises_count') !== 'exercises_count' ? t('exercises_count') : 'Exercises';
...
function renderExerciseList(filterText) {
    ...
    // Helper to get translated name/category for filtering
    const getTransName = (id, defaultName) => {
        const val = t(id);
        return val !== id ? val : defaultName;
    };

    const matches = exercises.filter(ex => {
        const transName = getTransName(ex.id, ex.name).toLowerCase();
        const transCat = getTransName(ex.category, ex.category).toLowerCase();
        // Translate equipment too
        const transEquip = getTransName(ex.equipment, ex.equipment).toLowerCase();

        return transName.includes(filterText) ||
            transCat.includes(filterText) ||
            transEquip.includes(filterText);
    });
    ...
    matches.forEach(ex => {
        ...
        const displayName = t(ex.id) !== ex.id ? t(ex.id) : ex.name;
        const displayCat = t(ex.category) !== ex.category ? t(ex.category) : ex.category;
        const displayEquip = t(ex.equipment) !== ex.equipment ? t(ex.equipment) : ex.equipment;

        item.innerHTML = `
            <div>
                <div style="font-weight:bold; color:white;">${displayName}</div>
                <div style="font-size:0.8rem; color:var(--accent);">${displayCat} • ${displayEquip}</div>
            </div>
...
// Global listener to re-render when language changes
document.addEventListener('languageChanged', () => {
    // Re-render routines to update "Exercises" count text if we put it in variables
    renderRoutines();

    // Re-render current routine view if active
    if (!routineDetail.classList.contains('hidden') && currentRoutineId) {
        const routine = routines.find(r => r.id === currentRoutineId);
        if (routine) {
            viewRoutine(currentRoutineId); // Re-renders title, count, and table
        }
    }

    // Re-render exercise list if modal is open to update names
    if (!exerciseSelectorModal.classList.contains('hidden')) {
        const term = document.getElementById('selectorSearch').value.toLowerCase();
        renderExerciseList(term);
    }
});
...
window.exportBackup = function () {
    const data = {
        routines: JSON.parse(localStorage.getItem('titanRoutines')) || [],
        lastWeight: localStorage.getItem('titanLastWeight') || null,
        // Você pode adicionar mais dados aqui conforme o app crescer
    };
    ...
    reader.onload = function (e) {
        try {
            ...
            if (importedData.routines) {
                localStorage.setItem('titanRoutines', JSON.stringify(importedData.routines));
                routines = importedData.routines; // Atualiza a variável global
            }
            if (importedData.lastWeight) {
                localStorage.setItem('titanLastWeight', importedData.lastWeight);
            }

            alert("Backup restaurado com sucesso!");
            location.reload(); // Recarrega para aplicar os dados
        } catch (err) {
